...
当前版本先把立法机制落到系统内的规则 Profile，而不是仓库内的 constitution 文件。主策划在"策划立法"页面维护页面类型、实体类型、关系类型、Wiki 必填章节、表字段推断规则和质量红线；系统为每个 Profile 计算 hash，并在构建运行、资产包质量摘要、资产组件和发布 manifest 中记录使用的规则版本。

新构建只使用当前启用的 Profile。未知页面类型、未知实体类型、未知关系类型、方向不清的关系、需要人工确认的表字段关系，都会进入候选或审核任务；候选关系不会混入可信图谱。发布前仍保持不可变 release 机制，同时检查 blocking 任务，并在 manifest 中暴露 source、package、component、质量摘要和立法 Profile 信息。

规则变更不会让旧资产立即失效。旧资产继续保留原构建 hash；当当前启用 Profile 与资产包记录的 hash 不一致时，发布摘要会提示需要复审。Agent 反馈继续回流为 review task，同类 miss 达到 3 次后升级为 repeated_query，作为错误本候选处理。
