...
npm run tauri:dev        # 桌面应用开发：并行起后端 + Tauri 壳
...
- **Node 22+**：服务器使用内置 `node:sqlite`（`DatabaseSync`）。低版本 Node 无此模块。
- **环境变量**：`PORT`（默认 4174）、`HOST`（默认 0.0.0.0）、`KH_DATA_DIR`（默认 `./data`）、`KH_JWT_SECRET`（默认 `dev-secret-change-me`，生产必须覆盖）。
- **首次启动**会在 `data/` 下创建 `knowledge-hub.sqlite`（WAL 模式）并 seed 三个演示用户和示范数据；详见 README。
...
- **Seed 数据**：演示数据通过 `users` 表为空判断是否注入，二次启动不会重复 seed。`seedDemoEvidence` 用 `INSERT OR IGNORE` 兜底。删改 demo 字段时要兼容旧 sqlite 文件或清掉 `data/`。
- **node:sqlite 实验警告**：当前 Node 22 会输出 ExperimentalWarning，不要为此添加 `--no-warnings` 等隐藏措施。
