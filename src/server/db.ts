...
async function migrate(adapter: DatabaseAdapter, schema: string): Promise<void> {
  ...
  await adapter.exec(`
    ...
    ALTER TABLE ${p}agent_events ADD COLUMN IF NOT EXISTS cluster_key TEXT NOT NULL DEFAULT '';
    CREATE INDEX IF NOT EXISTS idx_agent_events_cluster ON ${p}agent_events(project_id, feedback_type, cluster_key, created_at DESC);
