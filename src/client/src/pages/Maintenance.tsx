...
import { Badge, Metric, Page, Tabs } from "../components/Atoms";
import { QualityGateAdmin } from "../components/QualityGateAdmin";

type MaintenanceTab = "quality" | "legacy";

export function Maintenance() {
  const [tab, setTab] = useState<MaintenanceTab>("quality");
    ...
    <Page title="高级维护" subtitle="给管理员和主开发者查看底层 ID、迁移、审计和调试入口。">
      <Tabs
        ...
        items={[
          ...
          { id: "legacy", label: "旧库导入" }
        ]}
      />
      <div className="tab-panel" key={tab}>
        {tab === "quality" && <QualityGateAdmin />}
        {tab === "legacy" && (
          <>
